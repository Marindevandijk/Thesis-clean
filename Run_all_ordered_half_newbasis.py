...
for exp in experiments:
    ...
    h1_end = n_raw // 2
    dpp_h1 = dpp[:h1_end]
    theta1_h1 = theta1[:h1_end]
    theta2_h1 = theta2[:h1_end]
    ...
    X_seg_h2, time_idx_seg_h2, segment_ids_seg_h2, _ = Build_segmented_data(
        dpp_h2, theta1_h2, theta2_h2
    )

    # first half
    X_h1_list.append(X_seg_h1)
    segment_ids_h1_list.append(segment_ids_seg_h1 + seg_offset_h1)
    time_idx_h1_list.append(time_idx_seg_h1 + time_offset_h1)

    seg_offset_h1 += segment_ids_seg_h1.max() + 1
    time_offset_h1 += time_idx_seg_h1.max() + 1

    # second half
    X_h2_list.append(X_seg_h2)
    segment_ids_h2_list.append(segment_ids_seg_h2 + seg_offset_h2)
    time_idx_h2_list.append(time_idx_seg_h2 + time_offset_h2)

    seg_offset_h2 += segment_ids_seg_h2.max() + 1
    time_offset_h2 += time_idx_seg_h2.max() + 1

X_h1 = np.vstack(X_h1_list)
segment_ids_h1 = np.concatenate(segment_ids_h1_list)
time_idx_h1 = np.concatenate(time_idx_h1_list)

X_h2 = np.vstack(X_h2_list)
...
i_h2 = np.random.randint(0,len(X_h2))
...
traj_sim_h2, key = Simulation(S_h2, x0_h2, dt=0.01, N_steps=500000, key=key)


traj_sim_h1_np = np.array(traj_sim_h1)
traj_sim_h2_np = np.array(traj_sim_h2)


np.savez(
    os.path.join(outdir, "stochastic_simulated_trajectories_halves.npz"),
    traj_sim_h1=traj_sim_h1_np,
    traj_sim_h2=traj_sim_h2_np,
...
axs[0].hist(traj_sim_h1_np[:, 0], alpha=0.4, density=True, label="First 50%", bins=100)
axs[0].hist(traj_sim_h2_np[:, 0], alpha=0.4, density=True, label="Last 50%", bins=100)
axs[0].legend()
axs[0].set_title(r"$d_{pp}$")

axs[1].hist(wrap_pi(traj_sim_h1_np[:, 1]), alpha=0.4, density=True, bins=100)
axs[1].hist(wrap_pi(traj_sim_h2_np[:, 1]), alpha=0.4, density=True, bins=100)
axs[1].set_title(r"$\theta_1$")

axs[2].hist(wrap_pi(traj_sim_h1_np[:, 2]), alpha=0.4, density=True, bins=100)
axs[2].hist(wrap_pi(traj_sim_h2_np[:, 2]), alpha=0.4, density=True, bins=100)

axs[2].set_title(r"$\theta_2$")

plt.tight_layout()
fig_path = os.path.join(outdir, "stochastic_simulation_distributions_halves.png")
plt.savefig(fig_path, dpi=300)
plt.close()

js_h1 = average_js_score(X_h1[:, 0], wrap_pi(X_h1[:, 1]), wrap_pi(X_h1[:, 2]), traj_sim_h1_np)
js_h2 = average_js_score(X_h2[:, 0], wrap_pi(X_h2[:, 1]), wrap_pi(X_h2[:, 2]), traj_sim_h2_np)
...
with open(os.path.join(outdir, "metadata.txt"), "w") as f:
    f.write(f"length of data : {X_all_halfs.shape}\n")
