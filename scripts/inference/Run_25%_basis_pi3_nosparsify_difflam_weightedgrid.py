...
def Run_Force_inference(X,time_idx,K,M,lam):
    ...
    def C_function2(x):
        ...
        s = 6
        front1 = smooth_gate((jnp.pi/3) - a1, s)
        back1  = smooth_gate(a1 - (2*jnp.pi/3), s)

        front2 = smooth_gate((jnp.pi/3) - a2, s)
        back2  = smooth_gate(a2 - (2*jnp.pi/3), s)
...
outdir = os.path.join(base_dir, "Results_reproduce", "All_fightbouts_quarters_pi3_nospars_diflam_weighted")
...
key = random.PRNGKey(0)
all_endpoints_q1, all_forces_q1, startpoints_q1, accept_rate_q1 = Find_endpoints(
    S_q1, X_q1, outdir, tag="q1", save_last_n=3000
)

all_endpoints_q2, all_forces_q2, startpoints_q2, accept_rate_q2 = Find_endpoints(
    S_q2, X_q2, outdir, tag="q2", save_last_n=3000
)

all_endpoints_q3, all_forces_q3, startpoints_q3, accept_rate_q3 = Find_endpoints(
    S_q3, X_q3, outdir, tag="q3", save_last_n=3000
)

all_endpoints_q4, all_forces_q4, startpoints_q4, accept_rate_q4 = Find_endpoints(
    S_q4, X_q4, outdir, tag="q4", save_last_n=3000
)
